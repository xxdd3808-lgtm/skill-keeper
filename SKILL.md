...
3. **更新(需要联网与用户确认)**:`skill-keeper updates` 暂存固定候选 → 大模型按 skill-vetter 清单安检 → `manage vet <plan_id> --verdict safe --evidence <可核查依据>` 记账(网页点击不构成安检) → `manage plan update --instance <iid>` → `manage apply`。安检为 warning 需 `--accept-warning` 二次确认;danger 直接废弃候选。远端 HEAD 之后怎么变都不影响已审查的固定候选,应用前绝不重新下载。
