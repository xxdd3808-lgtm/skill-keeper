...
**一键处理**:`report.py --serve`(macOS 也可双击项目根的 `启动技能报告.command`)→ 自动开浏览器。网页是两阶段:删除/恢复先 `POST /api/plan` 生成不可变计划(展示摘要+digest),确认后 `POST /api/apply` 执行;更新流程在计划建立后暂停,展示可复制的正式安检命令(`skill-keeper manage vet`,需要可核查证据)与续办执行按钮——**网页点击不构成安检**。安全边界:只绑 127.0.0.1、随机 token 常量时间比较、POST 校验 Origin、请求体上限 64 KiB、交互脚本经带 token 的同源 `/report.js` 加载,响应带 nosniff/no-referrer/DENY/CSP,脚本不使用 `unsafe-inline`/`unsafe-eval`(样式保留 style-src `unsafe-inline`,仅覆盖报告自产内联样式)。带 token 的 URL 是接受的取舍:仅本机可访问 + `Referrer-Policy: no-referrer`,但会留在本机浏览器历史,介意可无痕窗口打开。**静态打开 report.html 时按钮退化为复制等价的 plan 命令(只含 instance_id,绝不含目录名)**。
